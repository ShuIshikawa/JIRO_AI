...
def main():
    ...
    if not os.path.isdir('./resources/log'):
    ...
    X = np.array(pd.read_csv('./resources/preprocessed_data/data.csv').values)
    Y = np.array(pd.read_csv('./resources/preprocessed_data/target.csv').values).reshape(-1,)

    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.2, random_state=random_state)
    n_samples, n_features = X_train.shape

    # 各特徴量の分散を描画
    #plot_variance(X, './documents/pictures/variances.png')
    ...
    for clf_name in clf_names:
        ...
        if clf_name == 'krs':
            ...
            model.save('./resources/estimators/' + clf_name + '.h5')
